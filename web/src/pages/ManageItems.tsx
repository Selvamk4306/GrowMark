    ...
    <div className="max-w-6xl mx-auto space-y-6 relative">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-textSecondary">Add, edit or remove products</p>
        </div>
        <button
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-background/50 border-b border-border text-sm text-textSecondary uppercase tracking-wider">
                ...
                <th className="p-4 font-semibold">Price / Cost</th>
                <th className="p-4 font-semibold">Daily Target</th>
                <th className="p-4 font-semibold">Weekly Target</th>
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr><td colSpan={5} className="p-8 text-center text-textSecondary">No items found.</td></tr>
              ) : (
                items.map(item => (
                  <tr key={item.id} className="hover:bg-background/30 transition-colors">
                    ...
                    <td className="p-4">
                      ...
                      <div className="text-xs text-textSecondary">Cost: {formatCurrency(item.cost_price)}</div>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-border flex justify-between items-center bg-background/50">
              <h2 className="text-lg font-bold text-primary">{editingItem ? 'Edit Item' : 'Add New Item'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-textSecondary mb-1">Item Name</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-textSecondary mb-1">Selling Price (₹)</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-textSecondary mb-1">Cost Price (₹)</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-textSecondary mb-1">Daily Target</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-textSecondary mb-1">Weekly Target</label>
              ...
              <div className="pt-4 flex justify-end space-x-3">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 text-textSecondary font-semibold hover:bg-background rounded-xl transition-colors">Cancel</button>
                <button type="submit" className="px-6 py-2 bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 transition-colors shadow-sm">{editingItem ? 'Save Changes' : 'Add Item'}</button>
              </div>
            </form>
          </div>
        </div>
